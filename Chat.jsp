                        ...
                        <div class="col-lg-12" style="x-overflow: inherit;overflow-y: scroll;height: 400px;">
                            ...
                            <div class="candidate-list">
                                ...
                                <%
                                    ...
                                    try {
                                        PreparedStatement st = con1.prepareStatement("select DISTINCT(toemail) from acceptemail where fromemail=? and flag=?");
                                        st.setString(1, email1);
                                        st.setInt(2, 1);
                                        ResultSet rs = st.executeQuery();
                                        while (rs.next()) {
                                            String toemail = rs.getString("toemail");
                                            PreparedStatement sts = con1.prepareStatement("select UPPER(username),UPPER(category),UPPER(city),email from lawyers where email=?");
                                            sts.setString(1, toemail);
                                        ...
                                        <div class="align-items-center row">
                                            ...
                                            <div class="col-lg-5">
                                                <div class="candidate-list-content mt-3 mt-lg-0">
                                                    <h5 class="fs-19 mb-0">
                                                        <a class="primary-link" href="#"><%=rss.getString("UPPER(username)")%></a><span class="badge bg-success ms-1"><i class="mdi mdi-star align-middle"></i>4.8</span>
                                                    </h5>
                                                    <p class="text-muted mb-2"><%=rss.getString("UPPER(category)")%></p>
                                                    <ul class="list-inline mb-0 text-muted">
                                                        <li class="list-inline-item"><i class="mdi mdi-map-marker"></i><%=rss.getString("UPPER(city)")%></li>
                                                        <!--                                                    <li class="list-inline-item"><i class="mdi mdi-wallet"></i> $650 / hours</li>-->
                                                    </ul>
                                                    <div>
<!--                                                        <a href="chatmain.jsp?toemail=<%=rss.getString("email")%>&name=<%=rss.getString("UPPER(username)")%>" class="btn btn-primary" >Send</a>-->
                                                        <a href="Clientsession.jsp?toemail=<%=rss.getString("email")%>&name=<%=rss.getString("UPPER(username)")%>" class="btn btn-primary" >Chat</a>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-lg-4">
